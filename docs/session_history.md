...
### 完了（2026-07-24 日次ジョブ停止の原因特定と修正・段位正規化の一本化）（commit `3e6ebee`）

Grafana から「日次ジョブが26時間以上正常完了していない」アラートが3回（10:12 / 14:17 / 18:22）。NAS もコンテナも4日間 Up（healthy）で、止まっていたのはジョブの中身だった。

- [x] **原因**: `2026-07-24 08:00` の日次が `invalid literal for int() with base 10: 'Raijin'` で停止。`db.get_last_rank_before_date()` が DB の `my_rank` を素の `int()` に通していた。wank(バルクAPI) は整数・ewgf.gg は英語段位名を返すため本番DBの `my_rank` は型が混在（実測 **integer 101件 / text 461件**）
- [x] **7日ぶりのランクマが引き金**: `_fire_rank_alerts` は `if not today_battles: return` で守られており、試合ゼロの日（7/17〜7/23）は到達しない。7/23 21:05〜21:13 の4戦で初めて到達し、直前の最新バトル（7/22 のクイック・`'Raijin'`）を `int()` に通して落ちた。7/16 の最終戦もクイックだったため、いずれ踏むのは確定していた
- [x] **修正**: `config.normalize_rank()` を追加し段位番号への正規化を一本化（int / 英語段位名 / 数字文字列を受け、未知の段位名は例外でなく None）。`db.get_last_rank_before_date` と `main._fire_rank_alerts` の `current_rank` に適用、`embeds._rank_id` は同等実装を委譲して重複排除
- [x] 検証: **654 tests / cov 96.02% / mypy 0**・pre-push フック通過。テスト4件追加（英語段位名・未知段位名・混在ソース・bool 排除）
- [x] NAS デプロイ完了。**起動時キャッチアップ（7/10 実装）が初めて実戦で機能**——`[catchup] daily ジョブの取り逃しを検出` → 昨晩の4戦を Discord へ `投稿完了`
- [x] **有線不通の切り分け**: 作業中に WSL からも Windows からも NAS へ到達不能に。`ipconfig` で有線が APIPA(169.254.x)・DHCP 応答なし。ケーブル挿し直し・GW(RTX830) 再起動でも復旧せず、**USB LAN に差し替えたところ同一ケーブル・同一ポートで DHCP 取得成功** → オンボード NIC(Realtek Gaming GbE) の故障で確定。ルーター・ケーブル・ポート・DHCP は無実
- [x] 昇格の記録: ユーザーが 7/23 に **雷神(22) → 鬼神(23)** へ昇格（4戦で昇格）

**私の誤り2件（記録）**: ①DB の `my_rank` が4件とも 22 だったことから「段位は動いていない」と断定した——値の意味（対戦時点の段位＝昇格前）を確かめずに出した結論で、ユーザーの実体験が正しかった。②`Find-NetRoute` が一貫して Wi-Fi を返しているのを「奇妙だ」と書きながら、GW への 2ms 応答だけを見て「有線が復活した」と報告した。観測の不整合を残したまま結論を出さないこと。

---

### 完了（2026-07-24 夜 障害の収束確認・LLM プロバイダ順の是正・昇格当日検知が不成立と判明）

前セッションで「明朝の日次で解ける見込み」としていた3件は、いずれも同日夜の時点で解消済みだった。コード変更なし（`docs/session_history.md` のみ）。

- [x] **心拍復旧を確認**: `tekken_last_success_timestamp{daily}` が 21:31 JST に更新（`age=4538s`）。Grafana アラートの継続要因は消えた
- [x] **LLM コメント復活を確認**: 21:26 のコンテナ再起動でキャッチアップが走り、`gemma3:4b` が 82文字を生成・**評価 80/100**・Embed へ追記済み。「コンテナ内からだけ Ollama へ繋がらない」現象は再現しなくなった（NIC を USB LAN へ差し替えた後の再起動が効いた可能性が高いが、直接の因果は未確認）
- [x] **WSL ネットワーク復旧**: `wsl -l -v` が即応答（Stopped）、起動後 NAS へ ping 2.9ms・SSH 疎通。デプロイ経路が戻った
- [x] **`GEMINI_FIRST=true` → `false`**（NAS `.env` 12行目のみ変更・`.env.bak.20260724` にバックアップ・`docker compose up -d` で再起動・`[catchup] 取り逃したジョブなし` で二重投稿なし）。クォータ 0 の Gemini を毎回先に叩いて 429 を踏んでから Ollama へ落ちていた無駄を除去。実際の呼び出し順の確認は次回の日次ログ待ち
- [x] **昇格の当日検知（案A）は不成立と判明・取り下げ**:
  - `wank.wavu.wiki/player/<id>` の HTML を実取得して全要素を走査した結果、**段位はページに存在しない**。あるのは glicko2（`μ` / `σ²`）・キャラ別 games・last-seen のみ。「プロフィールから現在段位を取れる」という案Aの前提は私の思い込みだった
  - `api.ewgf.gg/external` は `/`→403、`/player/<id>`・`/players/<id>`・`/stats/<id>` はいずれも **500**。現在段位を返す口は確認できず
  - 段位が乗るのは対戦レコードだけ＝常に「対戦時点＝昇格前」。よって**現行データソースでは当日検知は不可能**。**案B（遅れて出る）で確定**——通知が消えるわけではなく、次にランクマをした日に届く

---

### 完了（2026-07-26 ネイティブLinux機の開発環境化・pre-push 不発の修正・実データ由来の投稿欠陥6件）（commit `9954f2f` `f748b29` `c0cb86b`）

スマホ経由開発（Remonote）の受け皿として、3台目のワークツリーであるネイティブ Linux 機（10.0.0.53）を開発機として成立させ、そのうえで実 DB を使った投稿の欠陥潰しまで実施した。

**A. NAS への SSH 経路を確立し、デプロイ手順を機種非依存にした**（`9954f2f`）

- 専用鍵 `~/.ssh/tekken_deploy`（ed25519・パスフレーズなし）を新規に切り、NAS の `/home/ubuntu/.ssh/authorized_keys` へ登録。WSL 機の鍵（`ubuntu@DESKTOP-580KM8J`）とは共存。機ごとに分けたのは片方だけ失効できるようにするため
- `~/.ssh/config` に `tekken-nas` エイリアス（`IdentitiesOnly yes` は必須。既定鍵 `id_ed25519` を先に提示して弾かれるのを防ぐ）
- CLAUDE.md / `.claude/commands/deploy.md` の `wsl bash -c "ssh -i ... ubuntu@10.0.0.254 ..."` を `ssh tekken-nas '...'` へ統一。Windows 側 CC のみ `wsl bash -c` で包む、という差分だけを明記（判別は `/proc/version` に `microsoft` を含むか）。deploy.md からは Windows 絶対パス `cd E:/work/tekken_bot` も削除
- ★**踏んだ罠**: 公開鍵の登録時に `sudo su` を挟んだため `~` が `/root` に化け、`/root/.ssh/authorized_keys` に入って効かなかった。さらに後続の掃除で `sudo -i` の行だけが落ち、相対パスの `sed -i /tekken-deploy/d .ssh/authorized_keys` が ubuntu のホームで走って**入れたばかりの鍵を削除**した。教訓は「破壊的コマンドは必ず絶対パスで書く」——前の行が落ちても `can't read` で止まるだけで済む
- ★**端末の表示破損**: ユーザー側で私の出力が壊れて表示され、それをコピーすると壊れたまま実行される症状（長い一行が丸ごと落ちる／行末一文字が欠ける／**空白がすべて `q` に化ける**）。長い文字列は LAN 越しの一時 HTTP サーバ（`python3 -m http.server`）＋ `curl` で運び、空白が化けるときは `${IFS}` を空白の代わりに使って回避した。原因は未究明で次回に持ち越し

**B. 開発ツールチェーンを sudo 無しで構築**

- `pip` すら無い素の Python 3.12 だった。`sudo` にパスワードが要るので apt を使えず、`venv --without-pip` → `get-pip.py` → `requirements-dev.txt` で構築。`.venv/` `venv/` を `.gitignore` へ追加
- GitHub 認証は `gh` に既存トークンがありながら、gh の想定プロトコルが ssh・origin が https・`credential.helper` 未設定で噛み合っていなかった。`gh auth setup-git` で解決（デバイスコード認証は不要だった。対話 TUI と格闘する前に `gh auth status` を見るべきだった）

**C. pre-push フックが一度も発火していなかった**（`f748b29`）

- `.githooks/pre-push` のモードが `100644` で、git が「hook was ignored because it's not set as executable」と明示して飛ばしていた。**検査ゼロで push が通る状態**で、防壁が機能していなかった。`git update-index --chmod=+x` で `100755` に修正（全クローンに効く。WSL 側も同状態だったはず。Windows 側は Git for Windows が実行ビットを見ないため発火していた可能性あり・未確認）
- インタプリタ選択を追加。リポジトリ内に `.venv` があれば優先する（システム Python に mypy/pytest が無い機があるため）。検査内容は CI と同一のまま
- 検証: ローカルの空リポジトリへ push し、発火（654 passed → push 許可）と停止（型エラーを仕込んで `exit 1`）の両方を実測

**D. 実データで判明した投稿の欠陥6件を修正**（`c0cb86b`）

NAS の `battles.db`（562戦・ranked 101 / quick 461）を `bot/db.py` → `bot/embeds.py` に通し、合成データでは見えなかった問題を潰した。

1. **レーティングトレンドの外挿暴走**（`stats.predict_rating_trend`）: 月次に「+1039/日」と出たが同月のレート変動は `+8`。レート付き試合は4戦・すべて 07-23 21:05〜21:13 で**観測窓482秒（8分）**。それを24時間へ引き伸ばしていた。`MIN_TREND_SPAN_SECONDS`（3日）未満はトレンドを出さない
2. **週次クイックで同じ表が2回**: 使用キャラ1体の週はキャラ別「相手段位別」が全体内訳と完全一致。2体以上のときだけ全体側を出す
3. **前期間比較が母数を無視**（`_build_prev_comparison`）: 138戦→76戦と半減した月に「勝利数 -49」とだけ出て急落に見えた。今月・前月の成績と試合数を併記し、差は勝率ポイント（-5.9pt）で示す
4. **対戦成績の並びが少数試行で埋まる**（`_matchup_matrix`）: 勝率降順のため1〜3戦100%が上位を占め、最多対戦の Asuka(9戦) が沈んでいた。対戦数降順（同数なら勝率昇順＝苦手が上）へ。上限12行、超過分は「ほか N キャラ（計 M 戦）」と集約
5. **試合行の鉄拳力重複**（`_power_part`）: 22戦の日に同じ値が22回。自分の絶対値は 💥 フィールドにあるので行では `(鉄拳力差 -46,228)` と差分のみ
6. **表の桁が揃わない**: Embed 本文はプロポーショナルフォントで空白パディングでは揃わない。表をコードブロックで囲って等幅にし、パディングを `len()` ではなく**表示幅**（East Asian Width）ベースへ。日次の列幅はその日の最長に合わせて動的に決定

- 7件目「ランク戦週次が成立しない週が多い」（ranked 18%）は欠陥ではなく実態の反映と判断し、**手を付けずに残した**。投稿の有無を変えるのは運用の判断
- 検査: mypy 0件 / **672 passed** / カバレッジ 95.96%。テスト12件追加。NAS デプロイ済み（`docker compose up -d --build`、起動ログ正常、キャッチアップ二重投稿なし）
- ★**未検証**: 6 の「崩れていた」は推定であって現物を見ていない。明日 08:00 の日次投稿で列が揃っているかを目視確認すること

---

### 完了（2026-07-28 段位別勝率の同格マーカーを記号統一）（commit `a5f4c22`）

- **クイック週次の振り返り価値**（★最優先だった件）に着手。方針どおり `build_quick_weekly_embed` の現物を卓上 `bot/battles.db` の quick 45戦で描画して見せ、私の分析を先に被せずユーザーに欠点を指してもらった。
- 指摘: 段位別勝率の同格が `(自分)` という**文字**で、格上🔺 / 格下🔻 と体系が揃わず分かりづらい。
- 対応: `_rank_winrate_breakdown` の同格マーカーを `(自分)` → `🟰` に統一。凡例2箇所（クイック週次・ランク週次）を `🏅 段位別勝率（🔺格上 / 🟰同格 / 🔻格下）` へ更新。docstring・該当テストも追従。
- **色の検討**: ユーザーは赤いマーカーを希望したが、赤いイコール絵文字は存在しない。赤にするには形を丸/四角へ崩すことになる（🔴/🟥 を候補提示）。比較の上でユーザーは **🟰 黒のまま**を選択。→ 「赤い同格記号は無い」は今後蒸し返さない。
- 検査: mypy 0件 / **672 passed** / カバレッジ 95.96%。NAS デプロイ済み（`docker compose up -d --build`、全コンテナ Up、bot/exporter は再起動直後で health: starting=正常）。
- 実物は日曜 21:00 の週次投稿で出る。

---

### 完了（2026-08-25 鉄拳王昇格の投稿欠落の調査・新キャラ Bob 追従・Chara#N 残骸の自動修復）（commit `0059c9a` `5e6e1ad`）

ユーザーからの「鉄拳王の昇格投稿が抜けた」という申告を起点に、原因を実データで確定させ、
派生して見つかった欠陥3件まで潰した。

**A. 昇格投稿の欠落は不具合ではなく既知の制約の顕在化**

- 実測: DB 最新は 8/19 20:56 の `my_rank=24`（武神）で停止。8/18 22:08 の勝利で 23→24 に昇格し、
  22:10 のレコードに 24 が乗っている（power も 189,181→194,244 に跳ねている）。8/19 は 24 のまま2戦。
- 8/19 08:00 のログに `段位変化: 雷神 → 武神` があり、**通知機構自体は正常**。8/20 実行では
  前日以前の最新も 24・当日も 24 のため「変化なし」と正しく判定していた。
- 8/20 以降は毎朝 `wank HTML: 0 件取得`。wank プレイヤーページを直接引いても最新は 8/19 20:56 で
  一致し、**取得漏れではなく本当に1戦もしていない**ことを確認。
- 結論: 鉄拳王(25)への昇格は 8/19 20:56 の最終戦で発生したが、段位は対戦レコードにしか乗らず
  その値は常に対戦時点＝昇格前。**昇格後の対戦が0戦のため rank=25 のレコードが存在せず検知不能**。
  2026-07-24 に確定した「案B（遅れて出る）」の帰結そのもの。

**B. 現在段位を取る口が新たに生えていないか再検分（1ヶ月ぶり）— 結果は変化なし**

- `api.ewgf.gg/external` に API キー付きで再挑戦: `/player/` `/players/` `/stats/` `/profile/` は
  いずれも **500**、`/external` 直下は 403。生きているのは `/battles/<id>` のみ。
- その `/battles` のレコードも `p2_dan_rank='Bushin'`（＝対戦時点の段位）。wank バルクAPI の
  生スキーマも全フィールド走査したが、対戦後の段位を持つフィールドは**存在しない**。
- → 当日検知は依然として不可能。ここは動かせないとユーザーに報告し合意。

**C. 派生して発見・修正した欠陥3件**

1. **新キャラ Bob(47) が未追従**: game_version 30201 の実対戦 7,495 件で ID47 が最多（1,507件≒10%）
   にもかかわらず `CHARA_NAMES` 未定義だった。名前は推測せず、**別プレイヤー6人・別対戦6件の
   wank HTML 表記が全一致**することを確認して `47:"Bob"` と確定（46 も同手順で `Kunimitsu` 確認）。
   既存 ID のシフト有無も 0/9/15/22/24/28/33/40/44/45 の10点抜き取りで検証し**全一致・シフトなし**。
2. **`Chara#N` 残骸で集計がキャラ分裂**: 新キャラ追加直後は名前未知のまま保存されるため、後から
   名前が判明してもレコードは `Chara#43` のまま残る。一回限りの UPDATE では新キャラのたび再発する
   ので、`db.repair_unknown_chara_names()` を追加し `main()` 起動時に自走させた。本番 DB の8件
   （Fahkumram 4 / Lidia 3 / Eddy 1）を **8→0** に修復済み。
3. **未学習キャラ警告の表示バグ**: `my_chara_id or opp_chara_id` と `my_chara or opp_chara` を
   別々に `or` で拾っていたため「自分のキャラIDに相手のキャラ名」というチグハグ表示になっていた
   （毎朝 `ID=45 (Miary Zo)` と出ていた正体）。`_unknown_chara_label()` に切り出して修正。

**D. 段位経路の列挙（遅延中に段位が飛ぶ穴を塞いだ）**

- 通知が次のランクマまで遅れる以上、その間に2段階上がると途中の段位が投稿から消える
  （24→26 なら「武神 → 鉄拳覇皇」となり鉄拳王が記録にも投稿にも残らない）。
- `build_rank_change_embed` で差が2以上のとき経路を列挙するようにした。
  実出力: `**武神** → 鉄拳王 → **鉄拳覇皇**` / `**雷神** → 鬼神 → 武神 → **鉄拳王**`。
  降格側も対称に動く。異常データ対策に上限8段（`RANK_PATH_MAX_STEPS`）。

**検査・デプロイ**: mypy 0 / **686 passed** / カバレッジ 96.01%（CI 同一コマンド）。
pre-push フック通過を確認して push、NAS デプロイ済み（bot/exporter とも Up）。

**手順上の教訓（次の私へ）**:
- 「デプロイすれば起動時に修復が走る」とユーザーに説明したが、実際には修復は `main()` 内＝
  daily job 実行時に走る。コンテナ起動だけでは動かず、catchup も「取り逃しなし」で素通りする。
  合意した結果を当日中に出すため、副作用の無い修復関数のみ `docker exec` で単独実行して 8→0 を
  確認した。**説明した挙動と実際の発火点がずれていた点は訂正済み。**
- Bash ツール（Git Bash）で PowerShell の here-string `@'...'@` を使い、コミットメッセージ先頭に
  `@` が混入した。push 前に `--amend` で修正。**ツールごとに構文が違うことを忘れるな。**

---

### 完了（2026-08-28 NAS 37時間ハングの原因特定・クイック未投稿の恒久対策）（commit `9b31505`）

発端はユーザーの「クイックマッチ結果が投稿されなかった。サーバー死んでますかね」。
**3つの別問題が重なっていた**ので、順に切り分けた。

**1. NAS の 37時間ハング（原因: USB ディスクの脱落）**

症状は半死状態だった: ping 応答あり / SSH は TCP 接続後に `kex_exchange_identification:
Connection reset`（5回再現）/ Grafana(3000)・Prometheus(9090) が Connection refused /
exporter(9877) は HTTP 200 を返すが `tekken_` メトリクスが **0 行**（`exporter.py` の
`collect()` が DB 例外を握りつぶすため空になる）。

当初 ディスクフル／メモリ枯渇を疑ったが**外れ**（`/` 33% / 7.6Gi 中 1.1Gi）。
真因は `journalctl -b -1` の末尾が **8/27 07:29:32 で途絶**していたこと:

```
Aug 27 07:27:50 kernel: usb 2-2.3: USB disconnect, device number 8
Aug 27 07:27:50 kernel: sd 2:0:0:0: [sdc] FAILED Result: hostbyte=DID_NO_CONNECT
Aug 27 07:29:32 kernel: usb 2-2.3: USB disconnect, device number 10
```

`/mnt/ssd_1` の USB ディスク（JMicron **152d:0583**、シリアル `DD5641988389C`）が
8/23 から I/O エラーを出し 8/27 朝に脱落。**ルートの sda も USB 接続**（`lsblk` で `TRAN=usb`）
のため同じ xhci バスを巻き込み、journald がログを書けず sshd も fork できない状態になった。
電源再投入後そのディスクは列挙されず、`fstab` の `nofail` のおかげで起動できた。

**デバイス名は再起動で振り直される**——障害時の sdc（`2-2.3`/JMicron）と現在の sdc
（`2-2.4`/ULT-Best `7825:a2a4`）は別物。一度この取り違えで誤った説明をし、`lsblk`＋
`fstab` の UUID 照合（`20f8ed9c-...` が実在しない）で訂正した。

**2. 投稿が無かった理由（ハングとは無関係）**

- wank は**ランク戦のみ収録**（`fetcher.py:80`）→ クイックは ewgf だけが経路
- ewgf 側は正常。他プレイヤー3名で確認し 8/26・**8/27 のクイックが載っていた**
- ewgf と DB を日別照合すると 7/14〜8/19 が**一件の狂いもなく一致**＝取りこぼしなし。
  よって 8月にクイックを打っていないことも確定（ユーザーの記憶は曖昧だった）
- 遅延の実績を旧ログから測定: **7/30 21:32 の対戦 → 8/1 08:00 に取り込み（約34時間）**。
  docstring の「24時間遅延」より長い。8/27 分が 8/28 時点で未着なのは想定内だった
  （一度「24時間過ぎているのに無い」と誤って異常視し、実績測定で訂正）

**3. 改名は無罪だった**

ユーザーから「プレイヤーネームを変えたせいかも、戻します」の申し出。wank の Name history が
`Duskward → 2026-08-19 20:56:28 JST` / `ExodusOverseer → 2026-04-06 21:36:30 JST` を記録して
おり、**改名時刻が最後の対戦と秒まで一致**（= その名前で最初に観測された対戦を刻む仕様）。
つまり 8/19 の一戦は既に改名後で、それが両ソースに正常取り込み済み＝収集は壊れていない。
Bot も wank も ewgf も Polaris ID で引いている。**戻す必要なしと判断し、戻さないよう進言**
（戻すと「戻したから直った」のか「単に遅延だった」のか判別できなくなるため）。

**恒久対策（本命）**

`daily_posts` を調べると記録は 4 件（8/19・8/18・7/23・4/06）で**全部ランク戦の日**だった。
日次投稿は `main.py:442` で常に「前日」だけを見るため、約34時間遅れて着くクイックは間に合わず、
**クイックのみの日は一度も投稿されたことがなかった**（今回に限った話ではない）。

- `config.py`: `BACKFILL_DAYS = 7`
- `db.py`: `get_unposted_dates(since_ts, before_date, player_name)` —
  battles にあるが daily_posts に無い JST 日付を古い順に列挙。当日は取りこぼしか進行中か
  区別できないため除外
- `main.py`: `_run_for_player(..., fetch=True)` に分離（`fetch=False` で外部取得を丸ごと飛ばす）。
  `_backfill_unposted()` を daily の通常処理後に実行。**素朴に `main(target_date=...)` を日数分
  回すと ewgf を無駄に叩く（レート制限 100/日）ため取得は1回のまま投稿対象日だけを広げた**。
  列挙が通常処理の後に走るので投稿済みの前日分は自然に除外され、二重投稿は構造的に起きない

本番データでのドライラン: `BACKFILL_DAYS` を 7/14/30/90 で試算し、**7 と 14 は対象ゼロ**、
30 で 7/30、**90 では 29 日分が一斉に飛ぶ**ことを確認して 7 を採用。

**検査・デプロイ**: mypy 0 / **700 passed**（686→700）/ カバレッジ 96.05%（CI 同一コマンド）。
pre-push 通過、NAS デプロイ済み・全コンテナ Up。

**手順上の教訓（次の私へ）**:
- PowerShell ツールに `{{.Names}}` を渡すと ScriptBlock と誤解されてパースエラー。
  Docker の `--format` を使うときは Bash ツール側で。さらに `wsl bash -c "..."` の
  二重クォート内では `$p` が展開されて消える——**複雑な引用が要るリモート実行は
  スクリプトファイルを書いて `ssh "bash -s" < file` で流す**のが確実だった。
- python ワンライナーのクォートを壊し、`sqlite3.connect("'")` を実行して
  `~/tekken_bot/'` という 0 バイトのゴミファイルを作った。気づいて削除・報告済み。
  **リモートで python を叩くならヒアドキュメントでファイルに書く。**
- NAS の `sudo` は非対話では通らない（`a terminal is required`）。CC の `!` 実行でも
  パスワード入力できないため、`sudo cp` も `sudo sed` も**実行されず cat だけが走った**。
  「出力が変わらない」で終わらせず、mtime とバックアップの有無で*実行されたか*を裏取りした。
